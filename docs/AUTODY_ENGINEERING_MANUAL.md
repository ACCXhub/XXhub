...
当前核心版本为 `1.4.0`。官方 Test Center 的模块版本为 `1.2.0`，模块 API 为 `1`，兼容范围为 `>=1.3.0,<2.0.0`。核心版本与模块版本独立演进；模块不需要机械地追随核心版本号。
...
设置页应分别显示 AutoDy 核心版本、官方模块版本和兼容状态。例如 AutoDy `1.4.0` 与 Test Center `1.2.0` 且范围 `>=1.3.0,<2.0.0` 是正常兼容组合，不表示模块“落后”。
...
不得移动、删除或强制更新已发布标签；不得强推；不得提交运行时数据、`.venv`、`node_modules`、日志、缓存、模块注册表或私有截图。现有已发布基线为 `v1.3.0`，后续版本需在 CI 通过后使用新标签。
