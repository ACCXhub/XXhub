...
- 当前源码版本线：**1.5.0**。
- `v1.5.0` tag 已存在，但正式 Release workflow 仍被 MSI lifecycle acceptance 阻断；Portable/MSI 候选构建和 privacy/package verification 已通过，canonical v1.5.0 assets **尚未正式发布**。
...
只有当 v1.5.0 Release 页面真正发布 `AutoDy-1.5.0-x64.msi`、checksum/Portable/manifest 且正式 workflow 全绿后，才把它视为正式升级资产。
...
- v1.5.0 资产尚未出现：继续使用最后已发布稳定版本，不下载 CI 中间包冒充 Release。
...
AutoDy 当前重点是关闭 v1.5.0 MSI lifecycle blocker并完成正式 Release。后续以阻断性可靠性、安全、安装/平台兼容维护为主；通用浏览器工作流研究不在本仓库扩展。
