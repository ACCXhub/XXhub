...
function Start-Or-ReuseService {
    ...
    $script:ManagedPid = $process.Id
    for ($attempt = 0; $attempt -lt 40; $attempt++) {
        Start-Sleep -Milliseconds 250
        $snapshot = Get-ServiceSnapshot
        if ($null -ne $snapshot -and $snapshot.Pid -eq $ManagedPid -and (Test-HealthyAutoDy $snapshot $expected)) { return $snapshot }
        if ($null -ne $snapshot -and $snapshot.Pid -eq $ManagedPid -and (Test-OwnedAutoDy $snapshot) -and $null -ne $snapshot.Modules) {
            $module = @($snapshot.Modules.modules | Where-Object { $_.id -eq "autody-test-center" }) | Select-Object -First 1
...
$open.add_Click({ try { Start-Or-ReuseService | Out-Null; Start-Process $Url } catch { [Windows.Forms.MessageBox]::Show($_.Exception.Message, "AutoDy") | Out-Null } })
